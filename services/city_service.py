...
# Local fallback in case of database access issues
FALLBACK_CITIES = {
...
def get_all_cities():
    """
    Retrieves sorted list of all city names from the database,
    falling back to a local pre-seeded list if the connection fails.
    """
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT city_name FROM cities ORDER BY city_name ASC")
        ...
        print(f"Error fetching cities from database: {e}. Using local fallback.")
    
    return sorted(list(FALLBACK_CITIES.keys()))
def get_city_coordinates(city_name):
    """
    Gets latitude and longitude for a given city name.
    """
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute(
            "SELECT latitude, longitude FROM cities WHERE city_name = %s",
            (city_name,)
        )
        ...
        print(f"Error fetching city coordinates: {e}. Using local fallback.")
        
    return FALLBACK_CITIES.get(city_name, (0.0, 0.0))
def get_city_id(city_name):
    """
    Retrieves the database primary key ID of a city by its name.
    """
    try:
        conn = get_connection()
        cursor = conn.cursor()
    ...
    sorted_cities = sorted(list(FALLBACK_CITIES.keys()))
    if city_name in sorted_cities:
        return sorted_cities.index(city_name) + 1
    return None